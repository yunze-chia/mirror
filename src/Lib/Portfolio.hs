...
emptyPortfolio c = Portfolio { transactions = [], baseCurrency = c, holdings = M.empty }

insertTransaction :: Portfolio -> Transaction -> Portfolio
insertTransaction p t = p { transactions = newTransactions, holdings = newHoldings }
  where
    ...
    updateHoldings h (Transaction _ crVal cr drVal dr) =  addDebits . addCredits $ h
      where
